...
const ScreenShareModal: React.FC<ScreenShareModalProps> = ({ isOpen, onClose, onConfirm }) => {
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-xl animate-in fade-in duration-300 px-6">
      <div className="max-w-2xl w-full bg-[#0d0d0d] border border-white/5 rounded-sm p-10 shadow-2xl space-y-8 animate-in zoom-in-95 duration-300">
        <div className="flex flex-col items-center text-center space-y-4">
          <div className="w-12 h-12 rounded-xl bg-blue-600/10 flex items-center justify-center text-blue-500 mb-2 border border-blue-500/20">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
          </div>
          <h2 className="text-3xl font-bold text-white tracking-tighter">Choose what to share</h2>
          <p className="text-neutral-500 text-xs font-bold uppercase tracking-widest max-w-sm">
            Configure your presentation settings before broadcasting.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {modes.map(mode => (
            <button
              ...
              className={`flex flex-col items-center p-6 rounded-sm border transition-all text-center gap-4 ${selectedSurface === mode.id ? 'bg-blue-600/10 border-blue-500 text-white' : 'bg-white/5 border-transparent text-neutral-500 hover:bg-white/10'}`}
            >
              <div className={`p-3 rounded-lg ${selectedSurface === mode.id ? 'bg-blue-600 text-white' : 'bg-neutral-800'}`}>
                {mode.icon}
              </div>
              <div className="space-y-1">
                <span className="text-[11px] font-black uppercase tracking-widest block">{mode.label}</span>
                <p className="text-[9px] font-medium leading-relaxed opacity-60 line-clamp-2">{mode.desc}</p>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="pt-6 border-t border-white/5">
          <button 
            ...
            className={`w-full flex items-center justify-between p-5 bg-white/5 border rounded-sm transition-all text-left group ${shareAudio ? 'border-blue-500 bg-blue-500/5' : 'border-white/5 hover:border-white/10'}`}
          >
            <div className="flex items-center gap-4">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${shareAudio ? 'bg-blue-600 text-white' : 'bg-neutral-800 text-neutral-500'}`}>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg>
              </div>
              <div className="flex flex-col">
                <span className={`text-[10px] font-black uppercase tracking-widest ${shareAudio ? 'text-blue-500' : 'text-neutral-400'}`}>Include System Audio</span>
              </div>
            </div>
            <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-all ${shareAudio ? 'border-blue-500 bg-blue-500' : 'border-neutral-700'}`}>
              {shareAudio && <svg className="w-2 h-2 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={4} d="M5 13l4 4L19 7" /></svg>}
            </div>
          </button>
        </div>
