    ...
    <div className="flex flex-col lg:flex-row items-stretch w-full h-full bg-black overflow-hidden">
      ...
      <div className="flex-1 relative bg-neutral-950 flex items-center justify-center border-r border-white/5">
        {isVideoOff ? (
          <div className="flex flex-col items-center gap-8">
             ...
             <span className="text-xs font-light uppercase tracking-[0.5em] text-neutral-600 animate-pulse">Video Disabled</span>
          </div>
        ) : (
          <video 
            ref={videoRef} 
        ...
        <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-black/80 backdrop-blur-3xl px-8 py-4 border border-white/10">
          ...
          >
             <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
          </button>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[500px] flex flex-col p-16 space-y-12 animate-in slide-in-from-right-10 duration-700 bg-black">
        <div className="space-y-4">
          <h2 className="text-5xl font-light text-white tracking-tight uppercase leading-none">Ready for orbit.</h2>
          <p className="text-neutral-500 text-xs font-light uppercase tracking-[0.4em]">Joining <span className="text-white">{roomName}</span></p>
        </div>

        <div className="space-y-10">
          <div className="space-y-4">
            <label className="text-[10px] font-normal text-neutral-500 uppercase tracking-[0.3em]">Identity Signature</label>
            <input 
              type="text"
              value={userName}
              onChange={(e) => setUserName(e.target.value)}
              placeholder="ENTER_NAME"
              className="w-full bg-transparent border-b border-white/10 py-4 text-2xl text-white outline-none focus:border-white transition-all font-light tracking-normal"
              required
              autoFocus
            />
          </div>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-normal text-neutral-500 uppercase tracking-[0.3em]">Optic Source</label>
              <select 
                className="w-full bg-neutral-900 border border-white/5 py-4 px-4 text-[10px] font-normal text-white appearance-none outline-none focus:border-white cursor-pointer uppercase tracking-widest"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-normal text-neutral-500 uppercase tracking-[0.3em]">Audio Feed</label>
              <select 
                className="w-full bg-neutral-900 border border-white/5 py-4 px-4 text-[10px] font-normal text-white appearance-none outline-none focus:border-white cursor-pointer uppercase tracking-widest"
        ...
        <div className="flex flex-col gap-4 pt-10">
          <button 
            ...
            className="w-full py-6 bg-white hover:bg-neutral-200 disabled:bg-neutral-900 disabled:text-neutral-700 text-black text-xs font-normal uppercase tracking-[0.5em] transition-all shadow-xl active:scale-[0.99]"
          >
            Launch Session
          </button>
          <button 
            onClick={onCancel}
            className="w-full py-4 bg-transparent hover:bg-white/5 text-neutral-600 hover:text-white font-normal uppercase tracking-[0.3em] transition-all text-[10px]"
          >
            Abort
          </button>
        </div>
