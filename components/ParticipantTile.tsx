...
const ParticipantTile: React.FC<ParticipantTileProps> = ({ 
  ...
  const borderStyle = isSharing
    ...
    : isCurrentlySpeaking 
    ? 'border-2 border-white shadow-[0_0_40px_rgba(255,255,255,0.2)] z-10 scale-[0.998]' 
    : 'border border-white/5';

  const avatarSize = isStage ? 'w-48 h-48 text-7xl' : isSolo ? 'w-64 h-64 text-9xl' : isMini ? 'w-12 h-12 text-xl' : 'w-24 h-24 text-4xl';

  return (
    <div className={`relative bg-black overflow-hidden flex items-center justify-center transition-all duration-1000 w-full h-full group ${borderStyle}`}>
      
      {isCurrentlySpeaking && !isSharing && (
        <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden">
          <div className="absolute inset-0 border-[4px] border-white/10 animate-orbit-breathing opacity-50" />
        </div>
      )}
      ...
      <div className="absolute inset-0 overflow-hidden bg-neutral-950">
        {(!stream || participant.isVideoOff || participant.role === ParticipantRole.AI) ? (
          ...
          <video 
            ...
            playsInline
            muted={isLocal}
            className={`w-full h-full object-cover transition-all duration-1000 grayscale-[0.2] ${isLocal && participant.isMirrored ? 'scale-x-[-1]' : ''}`}
            style={{
      ...
      <div className={`absolute left-6 right-6 flex items-center justify-between z-20 ${isMini ? 'bottom-2 px-1' : 'bottom-6'}`}>
        <div className={`flex items-center gap-3 bg-black/90 backdrop-blur-3xl px-5 py-2.5 border border-white/10 transition-all duration-500 ${isMini ? 'scale-75 origin-left px-2 py-1' : ''}`}>
          <div className={`w-1.5 h-1.5 rounded-full ${isCurrentlySpeaking ? 'bg-white shadow-[0_0_8px_white]' : 'bg-neutral-800'}`} />
          <span className="text-white text-[10px] font-normal uppercase tracking-[0.3em]">{participant.name || 'ANON'} {isLocal ? '(YOU)' : ''}</span>
        </div>
      </div>
      ...
      <style>{`
        ...
        .animate-orbit-breathing { animation: orbit-breathing 4s ease-in-out infinite; }
      `}</style>
    </div>
  );
};
